    ...
    for page in range(last_page, 0, -1):
        ...
        for block in reversed(blocks[1:]):
            ...
            if media:
                ...
                return _prefer_sc(media, insert_user_id), post_date

    return None, None

# Members who embed YouTube videos for context/reference but whose actual creation
# is always on SoundCloud or HearThis — for these users SC/HT takes priority over YT.
SC_PREFERRED_USERS = {12718}  # irena-svetlovska

def _prefer_sc(media_list, insert_user_id=None):
    """For SC-preferred members, return the most recent SC/HearThis item if one exists.
    For all other members, return the last media item as normal."""
    if insert_user_id in SC_PREFERRED_USERS:
...
def get_most_recent_media_standard(disc_id, disc_body, insert_user_id=None):
    ...
    try:
        ...
        if chosen:
            return _prefer_sc(chosen, insert_user_id)
    except Exception as e:
        print(f"    Warning: failed to fetch comments: {e}")

    if not op_media:
        return None
    return _prefer_sc(op_media, insert_user_id)
